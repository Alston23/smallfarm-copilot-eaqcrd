...
const EQUIPMENT_TYPES = ['Tractor', 'Plow', 'Cultivator', 'Seeder', 'Harvester', 'Sprayer', 'Mower', 'Trailer', 'Hand Tools', 'Irrigation', 'Other'];
const LISTING_TYPES = ['For Sale', 'For Trade', 'For Rent'];
const CONDITIONS = ['Excellent', 'Good', 'Fair', 'Needs Repair'];

export default function CreateEquipmentListingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!title || !equipmentType || !description) {
      Alert.alert('Missing Information', 'Please fill in all required fields');
      return;
    }

    if (listingType === 'For Sale' && !price) {
      Alert.alert('Missing Price', 'Please enter a price for items for sale');
      return;
    }
    ...
    try {
      ...
      await authenticatedPost('/api/marketplace/equipment/listings', listingData);

      console.log('Equipment listing created successfully');
      Alert.alert('Success', 'Your equipment listing has been created!', [
        ...
        <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
              ...
              {EQUIPMENT_TYPES.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    { 
                      backgroundColor: equipmentType === type ? farmGreen : colors.card,
                      borderColor: equipmentType === type ? farmGreen : colors.border,
                    },
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.chipText,
                      { color: equipmentType === type ? '#fff' : colors.text },
                    ]}
                  >
                    {type}
                  </Text>
              ...
              {LISTING_TYPES.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    { 
                      backgroundColor: listingType === type ? farmGreen : colors.card,
                      borderColor: listingType === type ? farmGreen : colors.border,
                    },
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.chipText,
                      { color: listingType === type ? '#fff' : colors.text },
                    ]}
                  >
                    {type}
                  </Text>
                ...
                {CONDITIONS.map((cond) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.smallChip,
                      { 
                        backgroundColor: condition === cond ? farmGreen : colors.card,
                        borderColor: condition === cond ? farmGreen : colors.border,
                      },
                    ]}
          ...
          <View style={styles.section}>
            ...
            <TextInput
              ...
              placeholder="Describe the equipment, its condition, features, and any other relevant details..."
              placeholderTextColor={colors.icon}
              multiline
              numberOfLines={6}
