...
export default function ExpenseSummaryScreen() {
  ...
  const loadExpenseData = useCallback(async () => {
    console.log('Loading expense summary data');
    setLoading(true);
    try {
      ...
      setSummary({ totalExpenses, byCrop, byCategory, byMonth });
    } catch (error) {
          ...
          {loading ? (
            ...
            <>
              ...
              <View style={[styles.totalCard, { backgroundColor: appleRed }]}>
                ...
                <Text style={styles.totalValue}>{formatCurrency(summary.totalExpenses)}</Text>
              </View>
                  ...
                  {Object.entries(summary.byCategory)
                    ...
                    .map(([category, amount], index) => (
                      <View key={index} style={[styles.itemCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                        ...
                        <Text style={[styles.itemValue, { color: appleRed }]}>{formatCurrency(amount)}</Text>
                      </View>
                    ))}
                </View>
              )}
              ...
              {Object.keys(summary.byCrop).length > 0 && (
                <View style={styles.section}>
                  <Text style={[styles.sectionTitle, { color: colors.text }]}>Expenses by Crop</Text>
              ...
              {Object.keys(summary.byMonth).length > 0 && (
                <View style={styles.section}>
                  <Text style={[styles.sectionTitle, { color: colors.text }]}>Expenses by Month</Text>
